...
<p align="center">
  <img src="{{ '/assets/module6-week3/intro.jpg' | relative_url }}" alt="Pipeline Diagram" width="750">
</p> 
...
Forward propagation is the process of passing data from the input layer → hidden layers → output layer to produce the final prediction. At each layer, the network performs two steps:
...
- $\eta > 0$: learning rate — controls the step size of the update
- $\frac{\partial L}{\partial \theta^{(t)}}$: gradient of the loss with respect to the parameter
- the "−" sign means we move in the **opposite** direction of the gradient (downward slope)

If $\eta$ is too large → the model oscillates, does not converge.
...
- Parameter update → reduce error in the direction of gradient descent
...
I hope this guide helped you understand Multi-layer Perceptrons better. Keep learning, keep building, and never stop exploring! 🚀
